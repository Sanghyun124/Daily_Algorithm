...
computer_index=random.choice(list(range(0,13)))
...
''')
    ...
    while True :
        ...
        if is_sea(player_attack_index-1):
            if player_attacked[player_attack_index-1]==True :
...
게임이 종료되었습니다! {round}라운드 만에 플레이어의 승리입니다.''')
    ...
    computer_attack_range=list(i for i in range(0,15) if computer_attacked[i]==False)
